...
class CatchBot:
    ...
    def start(self):
        logger.info("{} | Start catching", self.info.symbol)
        ...
        while True:
            try:
                ...
                position = Position(order, self.info)
                    
                pnl = position.waitForPositionClosed()  # 포지션이 종료되면 리턴된다. (익절이든 본절/손절이든)
                logger.info("{} | 포지션 종료. PNL: {}%", self.info.symbol, pnl)
            
            except Exception as ex:
                logger.error("{} | CatchBot 종료. Exception: {}", self.info.symbol, repr(ex))
                break
    ...
    def waitForBuyClosed(self, order, waitSeconds):
        ...
        for sec in range(waitSeconds):
            o = Lib.api.fetch_order(order['id'], self.info.symbol)
    ...
    def getTargetDIPrice(self, curPrice):
        ma20 = Lib.get20Ma(self.info.symbol, curPrice)
        targetPrice = ma20 * (1 - self.info.targetDI)
        return targetPrice

    def orderBuyLimit(self, price):
        quantity = Lib.getQuantity(price, self.info.marginRatio)
        return Lib.api.create_limit_buy_order(self.info.symbol, quantity, price)
    ...
    def waitForCatch(self):
        ...
        while True:
            ...
            try:
                ...
                curPrice = Lib.getCurrentPrice(self.info.symbol)
                ...
                if (targetPrice * 1.003) >= curPrice:
                    ...
                    logger.info("{} | 캐치 매수 주문: {}", self.info.symbol, targetPrice)
                    beepy.beep(sound="coin")
                    order = self.waitForBuyClosed(order, WAIT_SECONDS_FOR_BUY)
                    
                    if Lib.hasClosed(order):
                        logger.info("{} | 매수 체결: {}", self.info.symbol, order['price'])
                        break   # 매수 체결되었으므로 캐치 종료
                    else:
                        logger.info(str(WAIT_SECONDS_FOR_BUY) + "초 동안 미체결되어 매수 취소함")
                        Lib.api.cancel_order(order['id'], self.info.symbol)  # 30초 동안 체결되지 않으면 주문 취소한다.
