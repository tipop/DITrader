...
class CatchBot:
    ...
    def getFilledPrice(self, order):
        ...
        logger.info("{:10} | 캐치 매수 체결됨. 체결가: {:10.5}\t크기: {:10}USDT", order['symbol'], entryPrice, usdtSize)
        TelegramBot.sendMsg("{:10} | 캐치 매수 체결됨. 체결가: {:10.5}\t크기: {:10}USDT".format(order['symbol'], entryPrice, usdtSize))
        return entryPrice
