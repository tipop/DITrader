...
class Position:
    ...
    def getPNL(self):
        ...
        if self.profitOrder != None and self.profitOrder['status'] == 'closed':
            ...
            logger.info("{:10} | 익절 완료. PNL: {:10.2f}", self.symbol, pnl)
            ...
            logger.info("{:10} | 본절 완료. PNL: {:10.2f}", self.symbol, pnl)
        ...
        while True:
            try:
                ...
                if self.stopOrder == None and self.isStopTriggerPriceOver():
                    ...
                    logger.info("{:10} | 본절로스 주문: {:10.5f}", self.symbol, self.stopOrder['price'])

                # 익절 체결됐나
                if self.hasProfitOrderClosed():
                    if self.stopOrder != None:
                        Lib.api.cancel_order(self.stopOrder['id'], self.symbol)
                        logger.info("{:10} | 스탑로스 취소: {:10.5f}", self.symbol, self.stopOrder['price'])
                    break
                ...
                if self.stopOrder != None and self.hasStopOrderClosed():
                    if self.profitOrder != None:
                        ...
                        logger.info("{:10} | 매도 취소: {:10.5f}", self.symbol, self.profitOrder['price'])
                    break
